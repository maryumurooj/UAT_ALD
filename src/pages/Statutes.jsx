  ...
  const Statutes = () => {
    ...
    useEffect(() => {
      setBottomHtmlContent(<LegislationInfo legislationInfo={Legislation} searchTerm={searchTerm} bareActName={bareActName} />);
    }, [Legislation, searchTerm, bareActName]);
    ...
    useEffect(() => {
      setBottomHtmlContent(
        <LegislationInfo legislationInfo={Legislation} searchTerm={searchTerm} bareActName={bareActName} />
      );
    }, [Legislation, searchTerm, bareActName]);
    
    useEffect(() => {
      if (searchTerm) {
        const likeMatch = (text) => new RegExp(`\\b${searchTerm}\\b`, 'i').test(text);
    
        const filteredNotes = Legislation?.shortNotes?.filter((shortNote) => {
          const containsTerm =
            likeMatch(shortNote?.shortNoteText) ||
            shortNote?.shortNoteParas?.some((para) => likeMatch(para?.shortNoteParaText));
    ...
    const updateCurrentItems = (pageNumber) => {
      ...
      setBottomHtmlContent(<LegislationInfo legislationInfo={{ shortNotes: itemsToDisplay }} searchTerm={searchTerm} />);
    };
    ...
    const updateTopHtmlContent = (index) => {
      ...
      setBottomHtmlContent(<LegislationInfo legislationInfo={Legislation} searchTerm={searchTerm} />);
      scrollToTop(); // Scroll to the top when content is updated
    };
    ...
    const handleSectionClick = (section) => {
      if (section === "Citations") {
        updateBottomHtmlContent(<LegislationInfo legislationInfo={Legislation} searchTerm={searchTerm} />);
    ...
    const handleClearAll = () => {
      setTopHtmlContent('');
      setBottomHtmlContent(<LegislationInfo legislationInfo={''} searchTerm={searchTerm} />); // Reset LegislationInfo to empty
      setHrefData('');
 ...
 const handleBareActUpdate = (bareActName) => {
  ...
  const handlePrint = () => {
    if (topHtmlContentRef.current) {
      const printContent = topHtmlContentRef.current.innerHTML;
      ...
      <div className={styles.fullcontainer}>
        ...
        <div className={styles.frame}>
          <div className={styles.topframe}>
            <FrontDBS handlePrint={handlePrint}/>
            <div className={`${styles.textStyles} ${styles.scrollableContent}`} ref={topHtmlContentRef}>
