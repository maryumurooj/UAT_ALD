...
const RearDashboard = forwardRef(({ results, onRowClick, onSaveToPad, judgmentCount, onClear, setIsLoading, selectedIndex, searchContext}, ref) => {
...
const dates = filteredResults
  .map(result => {
    if (!result.judgmentDOJ || result.judgmentDOJ.length !== 8) return null;
    return new Date(
...
if (!fromDate && !toDate) {
  ...
  dateRangeText = `${from} to ${to}`;
    ...
    notify.success(`${filteredResults.length} results saved to Pad with search context!`);
  } else {
...
useEffect(() => {
  const fetchCourts = async () => {
    try {
      const response = await fetch(`http://61.246.67.74:4000/api/uat/all-courts`);
      if (!response.ok) {
...
<div className={styles.paginationContainer}>
  ...
  {currentJudgmentCitation ? currentJudgmentCitation : "Judgment Citation"}
       ...
       <div className={styles.searchContainer}>
        ...
        <button className={styles.padButton} onClick={handleSaveToPadClick} title={`Save ${filteredResults.length} results to Pad with search context`}>
          Pad
        </button>
