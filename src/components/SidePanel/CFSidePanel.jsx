...
function SidePanel({ setResults, setJudgmentCount, setError, setSearchTerms, onSearch, fullCitation,
  setFullCitation, onClear, setIsLoading, onSearchContextChange}) 
  {
...
const generateAdvancedDisplayText = (searchData) => {
  ...
  if (searchData.sections?.length > 0) {
    const sectionTexts = searchData.sections.map(s => `${s.section} of ${s.act}`);
    filters.push(` ${sectionTexts.join(', ')}`);
  }
  
  if (searchData.subsections?.length > 0) {
    const subsectionTexts = searchData.subsections.map(s => 
      `${s.subsection} of ${s.section} of ${s.act}`
    );
  ...
  if (searchData.topics?.length > 0) {
   ...
   if (searchData.judges?.length > 0) {
    const judgeTexts = searchData.judges.map(judge => `Hon'ble Justice ${judge}`);
    filters.push(judgeTexts.join(', '));
  }
  
  // Updated advocates formatting - each name gets "Advocate" prefix  
  if (searchData.advocates?.length > 0) {
    const advocateTexts = searchData.advocates.map(advocate => `Advocate ${advocate}`);
  ...
  try {
    setIsLoading(true);
      ...
      const searchPromises = searchWords.map(word => 
          axios.post('http://61.246.67.74:4000/api/uat/freeword-search', { searchWords: [word] })
      );
      ...
      let combinedResults = searchResponses.flatMap(response => 
          response.data.filter(result => commonJudgmentIds.includes(result.judgmentId))
      );
      ...
      combinedResults.sort((a, b) => (b.judgmentCitation || '').localeCompare(a.judgmentCitation || ''));

      console.log('Unique results after filtering and sorting:', combinedResults.length);
  ...
  // Create cumulative search context BEFORE performing search
    createAdvancedSearchContext();
      ...
      if (!hasFreewordInput && hasSQLInputs) {
        ...
        const uniqueResults = Array.from(new Set(sqlResponse.map((result) => result.judgmentId))).map(
          (judgmentId) => sqlResponse.find((result) => result.judgmentId === judgmentId)
        );
      ...
      if (hasFreewordInput && hasSQLInputs) {
        const [sqlResponse, elasticResponse] = await Promise.all([runSQLSearch(), handleSearch()]);
        if (!Array.isArray(sqlResponse) || !Array.isArray(elasticResponse)) {
          throw new Error('Unexpected response format from SQL or Elasticsearch API.');
        }
        const sqlJudgmentIds = sqlResponse.map((result) => result.judgmentId).filter(Boolean);
        const elasticJudgmentIds = elasticResponse.map((result) => result.judgmentId).filter(Boolean);
        const commonJudgmentIds = sqlJudgmentIds.filter((id) => elasticJudgmentIds.includes(id));
        const combinedResults = [
          ...sqlResponse.filter((result) => commonJudgmentIds.includes(result.judgmentId)),
          ...elasticResponse.filter((result) => commonJudgmentIds.includes(result.judgmentId)),
        ];
        const uniqueResults = Array.from(new Set(combinedResults.map((result) => result.judgmentId))).map(
          (judgmentId) => combinedResults.find((result) => result.judgmentId === judgmentId)
        );
        results = uniqueResults;
        judgmentCount = uniqueResults.length;
        allSearchTerms = [
          ...
          ...subsections.map((s) => `${s.subsection} of ${s.section} of ${s.act}`),
          ...topics,
          ...judges,
          ...advocates,
          ...searchWords,
...
// Helper function to run the SQL search
  ...
  const runSQLSearch = async () => {
    ...
    const response = await fetch('http://61.246.67.74:4000/api/uat/searchAdvanced', {
      method: 'POST',
  ...
  useEffect(() => {
    const fetchWords = async () => {
      try {
        const response = await fetch('http://61.246.67.74:4000/api/uat/all-words');
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchLegislationNames = async () => {
      try {
        const response = await fetch('http://61.246.67.74:4000/api/uat/all-legislation');
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchJudges = async () => {
      try {
        const response = await fetch(`http://61.246.67.74:4000/api/uat/all-judge`);
        if (!response.ok) {
  ...
  useEffect(() => {
      const fetchTopics = async () => {
        try {
          const response = await fetch(`http://61.246.67.74:4000/api/uat/all-topic`);
          if (!response.ok) {
    ...
    useEffect(() => {
      const fetchAdvocates = async () => {
        try {
          const response = await fetch(`http://61.246.67.74:4000/api/uat/all-advocate`);
          if (!response.ok) {
    ...
    const fetchSections = async (legislationId) => {
      try {
        const response = await fetch(`http://61.246.67.74:4000/api/uat/sections?legislationId=${legislationId}`);
        if (!response.ok) {
  ...
  const fetchSubsections = async (legislationSectionId) => {
    try {
        const response = await fetch(`http://61.246.67.74:4000/api/uat/subsections?legislationSectionId=${legislationSectionId}`);
        if (!response.ok) {
...
const handleCitationSearch = async (selectedCitation) => {
  try {
    ...
    const response = await fetch(`http://61.246.67.74:4000/api/uat/searchByCitation?CitationText=${CitationText}`);
    if (!response.ok) {
      throw new Error('Network response was not ok');
    }
    const data = await response.json();
...
// Clear search context - ADD THIS
  ...
  <button onClick={handleAddSearchWord} className={styles.button}>+</button>
</div>
    ...
    <>
      {acts.map((act, index) => (
        <div key={index} className={styles.addedItem}>
          {act}
          <button className={styles.button} onClick={() => handleRemoveItem(index, setActs)}>
            -
          </button>
        </div>
      ))}
      ...
      {subsections.map((subsectionObj, index) => (
        <div key={index} className={styles.addedItem}>
          {subsectionObj.subsection} of {subsectionObj.section} of {subsectionObj.act}
          <button
            className={styles.button}
      ...
      {topics.map((topic, index) => (
        <div key={index} className={styles.addedItem}>
          {topic}
          <button className={styles.button} onClick={() => handleRemoveItem(index, setTopics)}>
            -
          </button>
        </div>
      ))}
      {judges.map((judge, index) => (
        <div key={index} className={styles.addedItem}>
          {judge}
          <button className={styles.button} onClick={() => handleRemoveItem(index, setJudges)}>
            -
          </button>
        </div>
      ))}
      {advocates.map((advocate, index) => (
        ...
        <div className={styles.BottomItems}>
          <div >
            <button className={styles.Searchbutton} onClick={handleAdvancedSearch}>
              Search
            </button>
          </div>
          <div>
